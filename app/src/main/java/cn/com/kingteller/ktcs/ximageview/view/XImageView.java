package cn.com.kingteller.ktcs.ximageview.view;
...
import cn.com.kingteller.ktcs.ximageview.R;
...
public class XImageView extends ImageView  {
    ...
    private boolean isMoveEnable = false; //是否可以移动

    private boolean isAutoScaleAnim = false; //是否自动缩放回复
    ...
    public XImageView(Context context, AttributeSet attrs) {
        ...
        isMoveEnable = ta.getBoolean(R.styleable.XImageView_isMoveEnable, false);
        isAutoScaleAnim = ta.getBoolean(R.styleable.XImageView_isAutoScaleAnim, false);
        ta.recycle();
        initUI();
    }
    ...
    void onTouchMove(MotionEvent event) {
        ...
        if (mode == MODE.DRAG) {
            ...
            //如果水平 或者垂直监控 并且可以移动
            if (isControl_H || isControl_V && isMoveEnable)
     ...
     * @param rotation
     */
    public void doRotate(float rotation){
    ...
    public abstract class OnMultiTouchListener implements OnTouchListener {
        ...
        public void removeCallback() {
            if (mRun != null) {
                getHandler().removeCallbacks(mRun);
                mRun = null;
            }
        }
        ...
        public boolean onTouch(final View v, final MotionEvent event) {
            if (event.getAction() == MotionEvent.ACTION_UP) {
                ...
                touchCount.incrementAndGet();//每点击一次就移除上一次的延迟任务，重新布置一个延迟任务
                removeCallback();
                ...
                postDelayed(mRun,getMultiTouchInterval());
            }
            return true;
        }
